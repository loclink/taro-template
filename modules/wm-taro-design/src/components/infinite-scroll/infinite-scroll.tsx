...
import { InfiniteScrollInstance, InfiniteScrollProps } from 'types';
import './index.less';

const clsPrefix = `wm-infinite-scroll`;
type IStatus = 'loading' | 'complete' | 'error';
let compInitIndex = 0;

function InfiniteScroll_(props: InfiniteScrollProps, ref: React.ForwardedRef<InfiniteScrollInstance>) {
  const {
    renderLoading,
    renderComplete,
    renderError,
  ...
  const _loadMore = useCallback(
    async (immediately?: boolean) => {
      if ((!onRequest && status === 'loading') || immediately) {
        setOnRequest(true);
        const statusValue = await loadMore();
        // 确保等待渲染任务执行完成
        setTimeout(() => {
          nextTick(() => {
  ...
  return (
    <View className={`${clsPrefix} ${clsPrefix}${compIndex} ${className}`} ref={thisDom}>
      {status === 'loading' && (
        <>
          {renderLoading || (
      ...
      {status === 'complete' && (
        <>{renderComplete || <View className={`${clsPrefix}-complete`}>{completeText}</View>}</>
      )}
      {status === 'error' && (
        <>
          {renderError || (
