...
import { CascaderOption, CascaderProps, CascaderValue, convertConfig } from 'types/cascader';
...
const InternalCascader = (props: CascaderProps) => {
  ...
  const initData = async () => {
    ...
    if (convertConfig && Object.keys(convertConfig).length > 0) {
      state.optionsData = convertListToOptions(options as CascaderOption[], convertConfig as convertConfig);
    } else {
  ...
  const syncValue = async () => {
    ...
    if (isLazy() && Array.isArray(currentValue) && currentValue.length) {
      needToSync = [];
      const parent: any = state.tree.nodes.find((node) => node.value === currentValue[0]);
      ...
      if (parent) {
        ...
        const last = await currentValue.slice(1).reduce(async (p: Promise<CascaderOption | void>, value) => {
          ...
          const node: any = parent?.children?.find((item: any) => item.value === value);
  ...
  return (
    <RootPortal enable>
      <View className={`${PREFIX}-cascader ${className}`} style={style}>
        <Popup
          className={`${PREFIX}-cascadar-popup`}
          show={visible}
          position='bottom'
          round
          closeable={closeable}
          ...
          onClose={closePopup}
        >
          <View className={`${PREFIX}-cascader__title`}>{title}</View>
          <Tabs
            active={tabvalue}
            animated={tabAnimate}
            renderNavLeft={
              <View className={`${PREFIX}-cascader-tab-box`}>
                {optiosData.map((pane, index) => (
                  <View
                    ...
                    className={`${PREFIX}-tabs__titles-item ${tabvalue === pane.paneKey ? 'active' : ''}`}
                    key={pane.paneKey}
                  >
                    <Text className={`${PREFIX}-tabs__titles-item__text`}>
                      {!state.initLoading && state.panes.length && pane?.selectedNode?.text}
                      {!state.initLoading && state.panes.length && !pane?.selectedNode?.text && '请选择'}
          ...
          >
            {!state.initLoading && state.panes.length ? (
                    ...
                    {pane.nodes &&
                      pane.nodes.map((node: any, index: number) => (
                        <CascaderItem
                          ...
                          id={`${PREFIX}-cascader-item${node.value}`}
                        />
                      ))}
                  </ScrollViewTimeout>
                </Tab>
              ...
              <Tab key='-999' active name='-999' lazyRender>
                <View className={`${PREFIX}-cascader-tab`} />
              </Tab>
            )}
          </Tabs>
        </Popup>
