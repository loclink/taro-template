...
function Index(props: CalendarProps, ref: React.ForwardedRef<ICalendarInstance>) {
  const {
    title = '日期选择',
    color,
    show,
  ...
  const getInitialDate = useCallback(
    function (defaultDate: any = null) {
      ...
      if (type === 'range') {
        ...
        const start = limitDateRange(startDay || now, minDate, getPrevDay(new Date(maxDate)).getTime());
        const end = limitDateRange(endDay || now, getNextDay(new Date(minDate)).getTime());
        return [start, end];
      }
      if (type === 'multiple') {
        if (Array.isArray(defaultDate)) {
  ...
  const initRect = useCallback(
    function () {
      ...
      contentObserver.current = contentObserver_;
      contentObserver.current.relativeTo(`.wm-calendar__body${compIndex}`);
  ...
  const emit = useCallback(
    function (date) {
      const getTime = (date: any) => (date instanceof Date ? date.getTime() : date);
  ...
  const bodyRender = () => {
    return (
      <View
        className={`wm-calendar ${className || ''}`}
      ...
      >
        {longspan && longSpanShow && poppable && (
          <LongSpan data={monthsData} current={currentMonthDate} setScrollIntoView={quickSwitchMonth} />
        )}
        <Header
          title={title}
          showTitle={showTitle}
        ...
        ></Header>
        <ScrollView
          className={`wm-calendar__body  wm-calendar__body${compIndex}`}
          scrollY
          ...
          {monthsData.map((item: any, index) => {
            return (
              <Month
                ...
                key={`wm-calendar-month___${index}`}
                id={`month_${item}`}
                className='month'
                date={item}
                type={type}
        ...
        >
          {showConfirm && (
            <VanButton
              ...
              type='primary'
              color={color}
              className='wm-calendar__confirm'
  ...
  return (
    ...
    <View catchMove>
      {poppable ? (
        <VanPopup
          className={'wm-calendar__popup--' + position}
          show={show}
          round={round}
          position={position}
      ...
      <VanToast id='wm-carlendar-toast'></VanToast>
    </View>
  );
}
