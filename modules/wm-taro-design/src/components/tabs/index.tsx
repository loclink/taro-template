...
export function Tabs(props: TabsProps) {
  ...
  const { scrollLeft, scrollable, currentIndex, skipTransition, scrollWithAnimation, lineOffsetLeft } = state;
  ...
  const trigger = function (eventName: 'onClick' | 'onChange' | 'onDisabled', child?: any) {
  ...
  const setCurrentIndexByName = function (name: any) {
    const matched = newChildren.filter((child: any) => (child.props.name || child.props.index) === name);
    if (matched.length) {
  ...
  return (
    <View
      className={`tabs-com-index${indexRef.current} ` + ' ' + utils.bem('tabs', [type] + ` ${className || ''}`)}
      style={style}
      {...others}
    >
      <View
