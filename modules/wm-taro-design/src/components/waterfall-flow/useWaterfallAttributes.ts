...
import { Gutter, WaterfallFlowColumn, WaterfallFlowItemAttributes, WaterfallFlowItemSize } from './types';
 ...
 * @param size 变化后的size
 */
export type OnResize<TRecord> = (item: TRecord, size: WaterfallFlowItemSize) => void;
/**
...
export const useWaterfallAttributes = <TRecord extends BaseRecord>(
  ...
  const columnsRef = useRef<WaterfallFlowColumn[]>([...Array(columnNum)].map(() => ({ height: 0, items: [] })));

  // 一个队列，用来存储各个元素的尺寸变化，然后统一计算更新，Taro.nextTick?
  const initItemsQueueRef = useRef<InitItem<TRecord>[]>([]);
  const itemResizeStatusRef = useRef<Map<string, ItemResizeStatus<TRecord>>>(new Map());
  ...
  const initItem = usePersistFn((item: TRecord, size: WaterfallFlowItemSize) => {
  ...
  const refreshColumns = usePersistFn((item: TRecord, size: WaterfallFlowItemSize) => {
    ...
    const { height = 0, ...otherProps } = attributesRef.current.get(item?.key) ?? ({} as WaterfallFlowItemAttributes);
    // 计算出当前项的高度和间隔的差值
    ...
    for (let i = index + 1; i < items.length; i++) {
      ...
      const nextItem = attributesRef.current.get(nextKey) ?? ({} as WaterfallFlowItemAttributes);
  ...
  const onResize = usePersistFn((item: TRecord, size: WaterfallFlowItemSize) => {
  ...
  const onRemove = usePersistFn((item: TRecord) => {
    ...
    const { items } = columnsRef.current[columnIndex]!;
    const index = items.indexOf(item?.key);
    const { top = 0, height: minusHeight = 0 } = attributesRef.current.get(item?.key) ?? {};
  ...
  const positionRecord: Record<string, WaterfallFlowItemAttributes> = Object.assign(
    {},
