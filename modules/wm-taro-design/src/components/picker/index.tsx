...
import { IPickerInstance, PickerChangeEvents, PickerProps } from 'types';
import { PREFIX } from '../wxs/bem';
import './index.less';

const Picker = forwardRef(function Index(props: PickerProps, ref: React.ForwardedRef<IPickerInstance>): JSX.Element {
  const {
    valueKey = 'text',
    toolbarPosition = 'top',
    defaultIndex,
            ...
            value: {
              picker: {
                ...
                setIndexes: (indexes: number[]) => {
                  const stack = indexes.map((optionIndex, columnIndex) => setColumnIndex(columnIndex, optionIndex));
  ...
  const setColumns = useCallback(
    function () {
      ...
      const stack = (columns_ || []).map((column, index) => setColumnValues(index, column.values));
      return Promise.all(stack);
    },
    [columns]
  );
  ...
  const setColumnValues = useCallback(function (index, options) {
    ...
    const isSame = JSON.stringify(column.props.options) === JSON.stringify(options);
    if (isSame) {
  ...
  const setValues = function (values: any) {
    const stack = values.map((value: any, index: number) => setColumnValue(index, value));
    return Promise.all(stack);
  };
  ...
  return (
    <View
      className={`${PREFIX}-picker  ${className}`}
      style={utils.style([style])}
      {...others}
      onTouchMove={onTouchMove_}
    >
      {toolbarPosition === 'top' && showToolbar && (
        <View className='wm-picker__toolbar toolbar-class'>
          <View
            className='wm-picker__cancel'
            hoverClass='wm-picker__cancel--hover'
            hoverStayTime={70}
            data-type='cancel'
            onClick={emit}
          >
          ...
          {title && <View className='wm-picker__title wm-ellipsis'>{title}</View>}
          <View
            className='wm-picker__confirm'
            hoverClass='wm-picker__confirm--hover'
            hoverStayTime={70}
            data-type='confirm'
            onClick={emit}
          >
      ...
      {loading && (
        <View className='wm-picker__loading'>
          <Loading color='#1989fa'></Loading>
        </View>
      )}
      <View
        className='wm-picker__columns'
      ...
      >
        {computed.columns(columns).map((item: any, index: number) => {
          return (
            <PickerColumn
              className='wm-picker__column column-class'
              key={`wm-picker__column_${index}column-class`}
              data-index={index}
              index={index}
              valueKey={valueKey}
        ...
        ></View>
        <View
          className='wm-picker__frame wm-hairline--top-bottom'
